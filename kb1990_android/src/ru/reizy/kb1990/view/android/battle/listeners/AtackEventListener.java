...
import static ru.reizy.kb1990.view.android.resource.Util.ANIMATION_SPEED;
...
public class AtackEventListener implements KBEventListener {
	...
	private void showAtacking(AtackInfo a) {
		...
		String action = a.isShooting() ? " стреляет " : a.isRevenge() ? " мстит " : " атакует ";
		...
		int d2 = a.getDead();
		if (a1 != -1) {
			view.showMessage(n1 + "(" + c1 + ")" + action + n2 + "(" + c2 + ")" + " на " + a1 + " убивая " + d2);
			try {
		...
		view.updateCells(false, a.getUnitLocation(), a.getAimLocation());
	}
}
